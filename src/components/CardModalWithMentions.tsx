...
export default function CardModalWithMentions(props: Props) {
  ...
  return <><CardModal {...props} /><aside className="mentions-panel" aria-label="Comentários com menções"><div className="mentions-panel-head"><div><div className="section-kicker">colaboração</div><h2>Comentários</h2></div><AtSign size={18} /></div><p>Use <strong>@Braga</strong>, <strong>@Pallus</strong> ou <strong>@Kayo</strong> para indicar alguém.</p><a className="drive-folder-link" href={getWorkspaceDriveUrl()} target="_blank" rel="noreferrer">Abrir pasta compartilhada do Google Drive</a><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Escreva um comentário com @menção..." rows={4} aria-label="Novo comentário com menção" />{suggestions.length > 0 && <div className="mention-suggestions" role="listbox">{suggestions.map((assignee) => <button key={assignee.id} onClick={() => addMention(assignee.name)}><span>@{assignee.name}</span><small>mencionar integrante</small></button>)}</div>}<button className="button primary" disabled={!text.trim()} onClick={addComment}><Check size={15} /> Adicionar comentário</button></aside></>;
}
