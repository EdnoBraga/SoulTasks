...
export default function TeamOverview({ session, config, members, presence, isAdmin, onInvite, onlineDurations = {}, onNotify, chatOpen, channels, messages, activeChannelId, onSelectChannel, onSend, onCloseChat, onOpenChat }: Props) {
  return <><CollapsibleSection sectionId="members" kicker="equipe interna" title="Membros" description="Presença, convite e chat da equipe" className="team-collapsible"><WorkspaceMembers members={members} presence={presence} isAdmin={isAdmin} onInvite={onInvite} onOpenChat={onOpenChat} />{chatOpen && <ChatPanel channels={channels} messages={messages} activeChannelId={activeChannelId} currentUserId={session.user.id} onSelectChannel={onSelectChannel} onSend={onSend} onClose={onCloseChat} />}</CollapsibleSection>{isAdmin && <CollapsibleSection sectionId="online" kicker="somente administrador" title="Tempo online" description="Duração de presença dos integrantes" className="team-collapsible"><div className="online-duration-panel" aria-label="Tempo online dos integrantes">{members.map((member) => <div className="online-duration-row" key={member.userId}><span>{member.displayName}</span><strong>{formatOnlineDuration(onlineDurations[member.userId] ?? 0)}</strong></div>)}</div></CollapsibleSection>}<CollapsibleSection sectionId="call" kicker="sala de equipe" title="Videochamada" description="Abra uma sala para a equipe" className="team-collapsible"><CallRoom session={session} config={config} members={members} onNotify={onNotify} /></CollapsibleSection></>;
}
