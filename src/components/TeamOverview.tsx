...
export default function TeamOverview({ session, config, members, presence, isAdmin, onInvite, onNotify, chatOpen, channels, messages, activeChannelId, onSelectChannel, onSend, onCloseChat }: Props) {
  return <section className="team-overview" aria-label="Equipe e sala de equipe"><WorkspaceMembers members={members} presence={presence} isAdmin={isAdmin} onInvite={onInvite} /><CallRoom session={session} config={config} members={members} onNotify={onNotify} />{chatOpen && <ChatPanel channels={channels} messages={messages} activeChannelId={activeChannelId} currentUserId={session.user.id} onSelectChannel={onSelectChannel} onSend={onSend} onClose={onCloseChat} />}</section>;
}
